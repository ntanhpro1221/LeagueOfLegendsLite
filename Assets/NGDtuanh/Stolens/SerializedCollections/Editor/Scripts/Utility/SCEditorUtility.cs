...
namespace AYellowpaper.SerializedCollections.Editor
{
    ...
    {
        public const float KeyValueTopPadding = 2f;
        public const float ValueSpace = 2f;
        ...
        public static float CalculateHeight(SerializedProperty property, bool drawAsList)
        {
            if (drawAsList)
            {
                float height = -ValueSpace;
                foreach (SerializedProperty child in GetChildren(property))
                    height += EditorGUI.GetPropertyHeight(child, true) + ValueSpace;
