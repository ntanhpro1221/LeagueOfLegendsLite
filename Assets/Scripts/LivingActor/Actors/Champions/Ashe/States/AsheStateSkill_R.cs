...
public static partial class AsheStateSkill_R {
    ...
    public partial struct Exit : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var (
                ...
                if (SMHelpers.TryExit<Skill_R_State>.ItemCommon(filter, common, commonChamp, stateData.ValueRO, selectLookup, curTick)) {
    ...
    public partial struct Enter : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var (
              ...
              , RefRW<RotationData>>()) {
                anim.SetAnim(SharedAnimKey.Skill_R);

                var animTick = animData.ValueRO.AnimLengthTicks[SharedAnimKey.Skill_R];
...
public static partial class AsheStateSkill_R {
    public partial struct Exit {
        private readonly partial struct StateFilterAspect : IAspect, IStateExitAspect<AsheTag, Skill_R_State> {
            ...
            private readonly EnabledRefRW<Skill_R_State>     _curStateEnable;

            RefRO<AsheTag> IStateAspect<AsheTag, Skill_R_State>. Identity => _identity;
            RefRO<Simulate> IStateAspect<AsheTag, Skill_R_State>.Simulate => _simulate;

            EnabledRefRW<StateNotExitedYet> IStateExitFunc<Skill_R_State>.StateNotExitedYet => _stateNotExitedYet;
            EnabledRefRW<Skill_R_State> IStateExitFunc<Skill_R_State>.    CurStateEnable    => _curStateEnable;
        }
    }

    public partial struct Enter {
        private readonly partial struct StateFilterAspect : IAspect, IStateEnterAspect<AsheTag, Skill_R_State> {
            private readonly RefRO<AsheTag>       _identity;
            private readonly RefRO<Skill_R_State> _curState;
            private readonly RefRO<Simulate>      _simulate;

            private readonly EnabledRefRO<StateRequireEnter> _stateRequireEnter;

            RefRO<AsheTag> IStateAspect<AsheTag, Skill_R_State>.      Identity => _identity;
            RefRO<Skill_R_State> IStateAspect<AsheTag, Skill_R_State>.CurState => _curState;
            RefRO<Simulate> IStateAspect<AsheTag, Skill_R_State>.     Simulate => _simulate;

            EnabledRefRO<StateRequireEnter> IStateEnterAspect<AsheTag, Skill_R_State>.StateRequireEnter => _stateRequireEnter;
        }
    }

    public partial struct Update {
        private readonly partial struct StateFilterAspect : IAspect, IStateAspect<AsheTag, Skill_R_State> {
            private readonly RefRO<AsheTag>       _identity;
            private readonly RefRO<Skill_R_State> _curState;
            private readonly RefRO<Simulate>      _simulate;
